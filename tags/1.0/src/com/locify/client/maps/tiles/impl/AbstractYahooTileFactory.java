...
public abstract class AbstractYahooTileFactory extends AbstractTileFactory {
     ...
     */
    public AbstractYahooTileFactory(MapLayer canvasToRepaintWhenLoaded, String provider, String mode, String baseUrl) {
    ...
    protected String getCoordinatePart(int x, int y, int zoom) {
        int tilemax = getInfo().getMapWidthInTilesAtZoom(zoom);
        return "&x=" + x +
                "&y=" + (tilemax / 2 - y - 1) +
                "&z=" + getParticularZoomFromReferenceZoom(zoom);
    }

    protected int getParticularZoomFromReferenceZoom(int referenceZoom) {
        return 26 - referenceZoom;
    }

    protected int getReferenceZoomFromParticularZoom(int particularZoom) {
        return 26 - particularZoom;
    }
}
