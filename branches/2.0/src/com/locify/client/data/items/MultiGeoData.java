...
//Logger.log("Add data: " + actualGeoData.toString());
        geoData.addElement(actualGeoData);
    }
...
//Logger.debug("  MultiGeoData.finalizeData() " + toString());
        try {
            ...
            for (int i = 0; i < geoData.size(); i++) {
                data = (GeoData) geoData.elementAt(i);
                ...
                if (data instanceof Route) {
                    ((Route) geoData.elementAt(i)).finalizeData();
                }
    ...
    public String toString() {
        ...
        for (int i = 0; i < geoData.size(); i++) {
            data += (geoData.elementAt(i).toString() + "\n");
        }
        data += " ... end of listing";
        return data;
    }
...
//Logger.debug("  MultiGeoData.getGeoData " + geoType + " " + index + "  actualData: " + toString());
    ...
    public void addItemLabels(Form form) {
        GeoData item;
        for (int i = 0; i < geoData.size(); i++) {
            item = (GeoData) geoData.elementAt(i);
            if (item instanceof Route) {
                Route route = (Route) item;
                form.addComponent(new Label(Locale.get("Route") + ": " + route.getName()));
                form.addComponent(new Label("  (" + route.pointCount + " " + Locale.get("Points") + ")"));
            } else if (item instanceof Waypoint) {
                Waypoint wpt = (Waypoint) item;
                form.addComponent(new Label(Locale.get("Waypoint") + ": " + wpt.getName()));
                form.addComponent(new Label("  "));
            } else if (item instanceof WaypointsCloud) {
                WaypointsCloud wptCloud = (WaypointsCloud) item;
                form.addComponent(new Label(Locale.get("Waypoint_cloud") + ": " + wptCloud.getName()));
                form.addComponent(new Label("  (" + wptCloud.getWaypointsCount() + " " + Locale.get("Points") + ")"));
            }
        }
    }
    ...
    public GeoData getGeoData(int index) {
        if (index < getDataSize()) {
            return (GeoData) geoData.elementAt(index);
        } else {
            return null;
        }
    }
