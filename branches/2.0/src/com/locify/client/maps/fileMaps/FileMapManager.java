...
//Logger.log("File path: " + (mapPath + createImageName(0, 0)));
    ...
    public synchronized boolean drawImageSingle(Graphics g, FileMapViewPort targetPort) {
        try {
            ...
            if (fileMapConfig.getMapViewPort().containsPosition(targetPort.getCenter())) {
                ...
                drawImageSingle(image, x1, x2, y1, y2, g, targetPort);
                return true;
            }
            return false;
        } catch (Exception e) {
    ...
    private boolean drawImageSingle(Image image, int x1, int x2, int y1, int y2, Graphics g, FileMapViewPort targetPort) {
        int width = x2 - x1;
        int height = y2 - y1;
        int to_x = 0, to_y = 0;
        ...
        try {
            ...
            g.drawImage(image.subImage(x1, y1, width, height, false), to_x, to_y);
