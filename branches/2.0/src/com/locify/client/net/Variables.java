    ...
    public static String replace(String text, boolean url) {
        try {
            ...
            if (text.indexOf("$client[width]") >= 0) {
                text = Utils.replaceString(text, "$client[width]", String.valueOf(Capabilities.getWidth()));
            }
            if (text.indexOf("$client[height]") >= 0) {
                text = Utils.replaceString(text, "$client[height]", String.valueOf(Capabilities.getHeight()));
            }
