...
/**
 * This class takes care about storing files on memory card or phone's memory.
 * @author Destil
 */
public class FileSystem {
    ...
    /** Cache open {@link FileConnection FileConnections}.
     *  Designed for an unsigned application running on the phones without the
     *  aid of mass confirmation of the registration / reading files permition. */
    private Hashtable fileConnectionCache;
    private Hashtable fileConnectionCounter;

    public FileSystem() {
        fileConnectionCache = new Hashtable();
        fileConnectionCounter = new Hashtable();
    }
...
//#             return null;
    ...
    private void incrementFileConnectionCounter(String fileUrl) {

        Integer counter = (Integer) fileConnectionCounter.get(fileUrl);
        if( counter == null ) {
            fileConnectionCounter.put(fileUrl, new Integer(1));
        } else {
            fileConnectionCounter.put(fileUrl, new Integer(counter.intValue() + 1));
        }
    }

    public synchronized FileConnection getFileConnection(String fileName) throws IOException {

        //#if fileInMemory
        //# createMemorySettings();
        //#endif
        
        final String fileUrl = getFileUrl(fileName);
        incrementFileConnectionCounter(fileUrl);
        
        FileConnection fc = (FileConnection) fileConnectionCache.get(fileUrl);
        if( fc == null ) {
    ...
    public synchronized String getFileConnectionStatus() {

        final String [] keys = new String [fileConnectionCounter.size()];
        final Enumeration keysEnum = fileConnectionCounter.keys();
        for( int i = 0; i < keys.length && keysEnum.hasMoreElements(); i++ ) {
        ...
        for( int i = 0; i < keys.length; i++ ) {
            for( int j = 0; j < keys.length; j++ ) {
                ...
                final Integer i1 = (Integer) fileConnectionCounter.get(keys[i]);
                final Integer i2 = (Integer) fileConnectionCounter.get(keys[j]);
        ...
        sb.append("File acces statistic:\n");
        for( int i = 0; i < keys.length; i++ ) {

            final String rootPrefix = getFileUrl(ROOT);
            final String key = ( keys[i].startsWith(rootPrefix) 
                    ? keys[i].substring(rootPrefix.length() > 1 ? rootPrefix.length() - 1 : 0 )
                    : keys[i] );
            final Integer value = (Integer) fileConnectionCounter.get(keys[i]);

            sb.append(value).append("x ").append(key).append("\n");
        }
...
//#             return null;
            ...
            FileConnection fileConnection = getFileConnection(folder);
...
//#             return null;
    ...
    public synchronized void writeTestData(String root) {
        try {
            //#if !applet
            FileConnection fileConnection = getFileConnection(root);
            fileConnection.close();
            //#endif
        } catch (Exception e) {
        }
...
//#             String locifyFolder = "Locify/";
            ...
            for (int i = 0; i < orderedRoots.size(); i++) {
                ...
                for (int j = 0; j < tryOuts.size(); j++) {
                    ...
                    try {
                        FileConnection fileConnection = getFileConnection(root + suffix);
                        if (!fileConnection.exists()) {
                        ...
                        //#if fileInMemory
                        //#  Logger.debug("Create memory file ...");
                        //#  return createMemorySettings();
                        //#else
                          Logger.debug("Normal call without memory file ...");
                          return true;
                        //#endif
                    } catch (Exception e) {
...
//#if fileInMemory
//#    private boolean createMemorySettings() {
//#         try {
//#             final String settingDirUrl = getFileUrl(ROOT + SETTINGS_FOLDER);
//#             if( !fileConnectionCache.containsKey(settingDirUrl) ) {
//#             FileMemoryConnection settingsDir = new FileMemoryConnection(settingDirUrl, true);
//#             settingsDir.mkdir();
//#             fileConnectionCache.put(settingDirUrl, settingsDir);
//#             Logger.debug("Create memory file: " + settingDirUrl);
//#             final String [] ONLY_IN_MEMORY = new String [] { SETTINGS_FILE, COOKIES_FILE,
//#               DELETED_FILE, SERVICES_FILE, MAINSCREEN_FILE, SERVICE_SETTINGS_FILE };
//#             for(int i = 0; i < ONLY_IN_MEMORY.length; i++ ) {
//#                 final String fileInMemory = getFileUrl(ROOT + ONLY_IN_MEMORY[i]);
//#                 final FileMemoryConnection fileInMemoryConnection = new FileMemoryConnection(fileInMemory, true);
//#                 settingsDir.addFile(fileInMemoryConnection);
//#                 fileConnectionCache.put(fileInMemory, fileInMemoryConnection);
//#                 Logger.debug("Create memory file: " + fileInMemory);
//#             }
//#             }
//#             return true;
//#         } catch (IOException ex) {
//#             Logger.error("FileSystem.init() # Create file system in memory: " + ex.toString());
//#                return false;
//#         }
//#   }
//#endif
...
//#             return null;
            //#else
            FileConnection fileConnection = getFileConnection(ROOT + folder);
            if (!fileConnection.exists()) {
                return new Vector();
            }
    ...
    public synchronized void delete(String fileName) {
        try {
            //#if !applet
            final String fileUrl = ROOT + fileName;
            FileConnection fileConnection = getFileConnection(fileUrl);
    ...
    public synchronized void deleteAll(String folder) {
        try {
            //#if !applet
            final String fileName = ROOT + folder;
            FileConnection fileConnection = getFileConnection(fileName);
            if (fileConnection.exists() && fileConnection.isDirectory()) {
                Enumeration files = fileConnection.list();
                fileConnection.close();
                while (files.hasMoreElements()) {
        ...
        try {
            ...
            if (!cacheClearing) {
                ...
                while (files.hasMoreElements()) {
                    ...
                    try {
                        final String fileName = ROOT + CACHE_MAP_TILE_FOLDER + file;
                        fc = getFileConnection(fileName);
...
//#             return false;
            ...
            if (fileName.startsWith("file://")) {
                fileConnection = getFileConnection(fileName); //TODO file cache!
            } else {
                fileConnection = getFileConnection(ROOT + fileName);
            }
    ...
    public synchronized void renameFile(String oldFile, String newName) {
        try {
            //#if !applet
            FileConnection fileConnection = getFileConnection(ROOT + oldFile);
            if (fileConnection.exists()) {
                fileConnection.rename(ROOT + newName); //TODO VERIFYME (original without ROOT prefix!
            }
            fileConnection.close();
            //#endif
        } catch (Exception e) {
    ...
    public synchronized long lastModifiedFile(String filePath) {
        try {
            //#if !applet
            FileConnection fileConnection = getFileConnection(ROOT + filePath);
            if (fileConnection.exists()) {
...
//#             return null;
            //#else
            //get the stream
            FileConnection fileConnection = getFileConnection(ROOT + fileName);
            if (!fileConnection.exists()) {
                return null;
            }
...
//#             return -1;
            //#else
            FileConnection fileConnection = getFileConnection(file);
            long size;
            if (!fileConnection.exists()) {
                return -1;
            } else {
    ...
    public String getLastChar(String filename) {
        try {
            //#if applet
//#             return null;
            //#else
        ...
        public void run() {
            synchronized (R.getFileSystem()) {
                try {
                    ...
                    //create if not exist
                    FileConnection fileConnection = R.getFileSystem().getFileConnection(ROOT + fileToWrite);
                    if (!fileConnection.exists()) {
    ...
    /** Support method for quick and quiet close {@link FileConnection}.
     *  @param connection if it is not null call {@link FileConnection#close()}
     *  @return <strong>true</strong> if connetion is null or close method without exception,
                <strong>false</strong> if catch {@link IOException}. Exception is logged as warn. */
    public static boolean quiteClose(FileConnection connection) {
        ...
        try {
            ...
            Logger.warning("FileSystem.quiteClose(FC): " + ex.toString());
            return false;
        }
    ...
    /** Support method for quick and quiet close {@link InputStream}.
     *  @param stream if it is not null call {@link InputStream#close()}
     *  @return <strong>true</strong> if connetion is null or close method without exception,
                <strong>false</strong> if catch {@link IOException}. Exception is logged as warn. */
    public static boolean quiteClose(InputStream stream) {
        ...
        try {
            ...
            Logger.warning("FileSystem.quiteClose(IS): " + ex.toString());
            return false;
        }
