...
//				}
		...
		{    
			if (TAG_A.equals(tagName))
			{
				...
				{
					//#style browserText
					linkItem = new StringItem(null, handleText(parser.getText()), de.enough.polish.ui.StyleSheet.browsertextStyle );
				}
				if (style != null) {
			...
			else if (TAG_INPUT.equals(tagName))
			{
				if (this.currentForm != null)
				{
					...
					{
						//#style browserInput
						TextField textField = new TextField((String)attributeMap.get("label"), value, 100, INPUTTYPE_TEXT.equals(type) ? TextField.ANY : TextField.PASSWORD, de.enough.polish.ui.StyleSheet.browserinputStyle );
						if (style != null) {
							textField.setStyle(style);
						}
						add(textField);
