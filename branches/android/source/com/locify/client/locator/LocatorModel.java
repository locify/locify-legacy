...
public class LocatorModel extends Thread implements LocationEventListener, LocationEventGenerator {
    ...
    public void load() {
        try {
            R.getLoading().setText(Locale.get(78));
            //acquire last location
            lastLocation = R.getSettings().getLastLocation();
            //provider adding
            providers = new Vector();
            if (Capabilities.hasJSR179()) {
                providers.addElement(new Provider("JSR179LocationProvider", Locale.get(70), "internal_gps_21x21.png"));
            }
            if (Capabilities.hasBluetooth()) {
                providers.addElement(new Provider("BTNMEALocationProvider", Locale.get(15), "bluetooth_gps_21x21.png"));
            }
            if (Capabilities.isWindowsMobile()) {
                providers.addElement(new Provider("TCPNMEALocationProvider", Locale.get(145), "pda_porter_21x21.png"));
            }
            if (Capabilities.isSonyEricsson() && Capabilities.hasCOMMs()) {
                providers.addElement(new Provider("COMNMEALocationProvider", Locale.get(224), "com_gps_21x21.png"));
            }
            //#if !release || applet
            providers.addElement(new Provider("SimulatorLocationProvider", Locale.get(139), "simulator_gps_21x21.png"));
        //#endif
        } catch (Exception e) {
...
//        }
    ...
    public boolean hasValidLocation() {
        return (getOwnState() == LocationProvider.READY || getOwnState() == LocationProvider.MANUAL || R.getContext().isTemporary());
    }
