...
//Logger.debug("  step1 (" + (System.currentTimeMillis() - time) + "ms)");
                        ...
                        Gauge gauge = new Gauge(Locale.get(493), true, files.size(), 0);
                        form.insert(0, gauge);
...
//Logger.debug("Dos write: " + point.getLatitude() + " " + point.getLongitude());
                                            ...
                                            dis.close();
                                            bais.close();
                                            form.insert(1, new StringItem(fileName, Locale.get(494) + " (" + (System.currentTimeMillis() - time) / 1000 + "s)"));
                                        } catch (IOException ex) {
                                            ex.printStackTrace();
                                        }
                                    } else {
                                        form.insert(1, new StringItem(fileName, Locale.get(348) + " (" + (System.currentTimeMillis() - time) / 1000 + "s)"));
                                    }
                                } else {
                                    form.insert(1, new StringItem(fileName, Locale.get(499) + " (" + (System.currentTimeMillis() - time) / 1000 + "s)"));
                                }
                                gauge.setValue(k + 1);
                            }
                            ...
                            if (STOP) {
                                form.insert(1, new StringItem(Locale.get(496), null));
                            }
                        } else {
                            form.insert(1, new StringItem(Locale.get(495), null));
                        }
                        try {
                            dos.flush();
                        } catch (IOException ex) {
                        ...
                        form.insert(1, new StringItem(Locale.get(489) + " (" + (System.currentTimeMillis() - time) / 1000 + "s)", ""));
