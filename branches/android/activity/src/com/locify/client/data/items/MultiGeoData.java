    ...
    public void addItemLabels(Form form) {
        ...
        for (int i = 0; i < geoData.size(); i++) {
            ...
            if (item instanceof Route) {
                ...
                form.append(new StringItem(Locale.get(472) + ": " + route.getName(),
                        "  (" + route.pointCount + " " + Locale.get(471) + ")"));
            } else if (item instanceof Waypoint) {
                Waypoint wpt = (Waypoint) item;
                form.append(new StringItem(Locale.get(473) + ": " + wpt.getName(),
                        "  "));
            } else if (item instanceof WaypointsCloud) {
                WaypointsCloud wptCloud = (WaypointsCloud) item;
                form.append(new StringItem(Locale.get(474) + ": " + wptCloud.getName(),
                        "  (" + wptCloud.getWaypointsCount() + " " + Locale.get(471) + ")"));
            }
        }
    }
