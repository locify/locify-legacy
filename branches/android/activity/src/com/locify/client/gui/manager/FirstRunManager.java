...
import de.enough.polish.android.rms.InvalidRecordIDException;
...
public class FirstRunManager implements CommandListener, ItemCommandListener {
    ...
    public void loadLanguage() {
        try {
            ...
            cmdContinue = new Command(Locale.get(30), Command.SCREEN, 2);
            cmdSet = new Command(Locale.get(332), Command.SCREEN, 1);
            cmdExit = new Command(Locale.get(49), Command.EXIT, 3);
            cmdHowto = new Command(Locale.get(331), Command.ITEM, 1);
            dis.close();
            recordStore.closeRecordStore();
        } catch (Exception e) {
    ...
    private void viewLanguageSelection() {
        lstLanguages = new List(Locale.get(213), List.IMPLICIT);
    ...
    public void viewPermissionsWarning() {
        frmPermissionWarning = new Form(Locale.get(83));
        //#style browserTextBold
        StringItem siWarning = new StringItem(null, Locale.get(334), de.enough.polish.ui.StyleSheet.browsertextboldStyle );
        frmPermissionWarning.append(siWarning);
        btnHowto = new StringItem(null, Locale.get(331), StringItem.BUTTON);
    ...
    private void viewRootSelection(String folder) {
        lstRoots = new List(Locale.get(333), List.IMPLICIT);
    ...
    private void viewPermissionsHelp() {
        frmPermissionsHelp = new Form(Locale.get(331));
        if (Capabilities.isNokia()) {
            StringItem siNokiaS60 = new StringItem(Locale.get(337), Locale.get(343));
            frmPermissionsHelp.append(siNokiaS60);
            StringItem siNokiaS40 = new StringItem(Locale.get(336), Locale.get(342));
            frmPermissionsHelp.append(siNokiaS40);
        } else if (Capabilities.isSonyEricsson()) {
            StringItem siSE = new StringItem(Locale.get(338), Locale.get(344));
            frmPermissionsHelp.append(siSE);
            StringItem siUIQ = new StringItem(Locale.get(349), Locale.get(350));
            frmPermissionsHelp.append(siUIQ);
        } else {
            StringItem siGeneral = new StringItem(null, Locale.get(339));
            frmPermissionsHelp.append(siGeneral);
        }
        StringItem siNotPossible = new StringItem(Locale.get(340), Locale.get(341));
...
//#             String locifyFolder = "Locify/";
                ...
                if (R.getFileSystem().createRoot(selected + locifyFolder)) {
                    ...
                    Alert alert = new Alert(Locale.get(47), Locale.get(2), null, AlertType.ERROR);
                ...
                if (R.getFileSystem().getFolders(selected).hasMoreElements()) {
                    ...
                    Alert alert = new Alert(Locale.get(152), Locale.get(335), null, AlertType.WARNING);
                    R.getMidlet().switchDisplayable(alert, lstRoots);
                }
