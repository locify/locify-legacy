...
import de.enough.polish.android.rms.RecordComparator;
...
public class FileSystemScreen implements CommandListener, Comparator {
    ...
    private Command cmdSortName = new Command(Locale.get(223), Command.SCREEN, 20);
    private Command cmdSortDistance = new Command(Locale.get(222), Command.SCREEN, 20);
    private Command cmdDeleteAll = new Command(Locale.get(40), Command.SCREEN, 30);
    private Command cmdMapAll = new Command(Locale.get(407), Command.SCREEN, 40);
...
//Logger.debug("---------- FILESYSTEM browser view -------");
            ...
            if ("location".equals(target)) {
                lstFiles = new List(Locale.get(132), Choice.IMPLICIT);
            } else if ("place".equals(filter)) {
                lstFiles = new List(Locale.get(397), Choice.IMPLICIT);
            } else if ("route".equals(filter)) {
                lstFiles = new List(Locale.get(398), Choice.IMPLICIT);
            } else {
                lstFiles = new List(Locale.get(383), Choice.IMPLICIT);
            }
...
//Logger.debug("waypoint name: " + waypoint.getName());
                ...
                if (filter == null) {
                    //#style imgDelete
                    lstFiles.addCommand(Commands.cmdDelete, de.enough.polish.ui.StyleSheet.imgdeleteStyle );
                    //#style imgDelete
                    lstFiles.addCommand(cmdDeleteAll, de.enough.polish.ui.StyleSheet.imgdeleteStyle );
                }
            ...
            //#style imgHome
            lstFiles.addCommand(Commands.cmdHome, de.enough.polish.ui.StyleSheet.imghomeStyle );
    ...
    public void commandAction(Command c, Displayable d) {
        if (c == Commands.cmdHome) {
            ...
            if (item.getType() == GeoFiles.TYPE_CORRUPT) {
                R.getCustomAlert().quickView(Locale.get(447), Locale.get(152), "locify://files");
                return;
            }
            if ("location".equals(target)) {
